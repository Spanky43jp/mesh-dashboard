...
if(!function_exists('wmpl_save_config_file')){
    function wmpl_save_config_file(){
        ...
        $wmpl_nzs_strings .= '<key name="theme_mods_'.$theme_name.'">'."\n";
...
if(!function_exists('ninezeroseven_check_theme_compat')){
    function ninezeroseven_check_theme_compat( $data ){
        $version_check = get_option('nzs_version_info');
        if(is_array($data) && count($data) > 10 && isset($data['smof_init']) && $version_check === false){
            update_option('nzs_version_info', THEMEVERSION );
            $data['nzs_style_change'] = 0;
    ...
    add_filter('of_options_after_load', 'ninezeroseven_check_theme_compat' );
}
?>