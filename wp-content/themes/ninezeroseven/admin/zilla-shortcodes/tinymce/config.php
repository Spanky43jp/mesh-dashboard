<?php
	...
	$post_term_args = array('taxonomy' => 'category');

	$filter_post_term_by = get_terms("category",$post_term_args);

	$post_term_shortcode_array = array();

	$post_term_shortcode_array['all'] = 'All Categories';

	foreach ($filter_post_term_by as $key) {

		$post_term_shortcode_array[$key->slug] = $key->name;
...
$zilla_shortcodes['blog'] = array(
	...
	'params' => array(
			...
			'std'          => '',
			'type'         => 'text',
			'label'        => __('Post Count', 'textdomain'),
			'desc'         => __('Changes the amount of posts shown, use -1 to show all posts.', 'textdomain')
		),
		'post_cat' => array(
			'type' => 'select',
			'label' => __('Category', 'textdomain'),
			'desc' => __('Use dropdown if you want just a single category', 'textdomain'),
			'options' => $post_term_shortcode_array
		),
		'blog_layout' => array(
		'type' => 'select',
		'label' => __('Layout', 'textdomain'),
		'desc' => __('Allows to change the layout to different than theme options.', 'textdomain'),
		'options' => array(
				'default'       => 'Default',
	...
	'shortcode' => '[blog post_count="{{num_of_posts}}" post_category="{{post_cat}}" layout="{{blog_layout}}"]',
	'popup_title' => __('Insert Blog Shortcode', 'textdomain')
);
...
$zilla_shortcodes['service'] = array(
	...
	'params' => array(
		...
		'icon' => array(
			...
			'desc' => __('Used if image path below is empty', 'textdomain'),
	...
	'shortcode' => '[nzs_service title="{{title}}" icon_url="{{icon_url}}" icon="{{icon}}" link="{{link_url}}"] {{content}} [/nzs_service]',
