...
*************************************************************************/
	...
	if(has_post_thumbnail() || 'video' == $portfolio_type){
		...
		$thumb = nzs_get_post_image(get_post_thumbnail_id(), true);
		...
		if('video' == $portfolio_type){
			...
			if(is_array($meta)){
				...
				echo '<div class="post-slider slider-'.get_the_ID().'" data-slider-speed="'.esc_attr($flexslider_speed).'" data-slider-start="'.esc_attr($flexslider_auto).'">';
				echo '<div class="featured-post-content-wrapper">';
				echo '<div class="image-wrapper">';
				echo '<ul class="slides">';

				printf('<li><a href="%1s" title="%2$s" rel="prettyPhoto[%3$s]" class="img-preview photo-box"><img src="%4$s" alt="%3$s" class="scale-with-grid" width="%5$s" height="%6$s"/></a></li>',
				esc_attr($large_image_url[0]),
				esc_attr($image_caption),
				esc_attr(the_title_attribute('echo=0')),
				esc_attr($thumb),
                ...
                foreach ($meta as $key => $option) {
                    ...
                    if($image_attributes){
                    	...
                    	printf('<li><a href="%1$s" title="%3$s" rel="prettyPhoto[%4$s]" class="img-preview"><img src="%2$s" alt="%4$s" class="scale-with-grid" width="%5$s" height="%6$s"/></a></li>',
                    		$full_size_attributes[0],
                    		esc_attr($image_attributes),
                    		esc_attr($image_caption),
                    		the_title_attribute('echo=0'),
                    		$full_size_attributes[1],
                    		$full_size_attributes[2]
                    		);
                    }
				...
				echo '<a class="photo-up hide-text"></a>';
				...
				$external_url = get_post_meta( get_the_ID(), 'nzs_external_link', true);
					...
					if(isset($external_url) && !empty($external_url)){
						...
						echo '<a href="'.esc_attr($external_url).'" target="_blank" class="web-link hide-text"></a>';

					}
				}
				
				if(!is_single()){
					echo '<a href="'.get_permalink().'" class="go-link hide-text"></a>';
				}
                ...
                echo '</div>';
       			...
       			printf('<a href="%1s" title="%2$s" rel="prettyPhoto" class="img-preview photo-box"><img src="%4$s" alt="%3$s" class="scale-with-grid" width="%5$s" height="%6$s"/></a>',
				esc_attr($large_image_url[0]),
				esc_attr($image_caption),
				esc_attr(the_title_attribute('echo=0')),
				esc_attr($thumb),
			...
			echo '<span class="extra-links">';
			echo '<a href="'.$large_image_url[0].'" title="'.esc_attr(the_title_attribute('echo=0')).'" rel="prettyPhoto['.esc_attr(the_title_attribute('echo=0')).']" class="photo-up hide-text"></a>';
