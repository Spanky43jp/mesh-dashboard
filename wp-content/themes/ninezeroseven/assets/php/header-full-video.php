...
if(!function_exists('fullscreen_init')){
	function fullscreen_init(){
		...
		if( 1 === preg_match('/youtube.com\/embed\/([^\/]+)/', $video_link , $matches)){
	     ...
	     $nzs_video_quality = '';
	     if(isset($smof_data['nzs_video_quality']) && $smof_data['nzs_video_quality'] != 'default'){
	     	$nzs_video_quality = ',quality:\''.esc_html($smof_data['nzs_video_quality']).'\'';
	     }
	?>
	<script src="//www.youtube.com/iframe_api"></script>
	<script type="text/javascript">
		(function(){
		   jQuery('#full-youtube-video-container').tubular({videoId: '<?php echo $video_url;?>',repeat:<?php echo $video_repeat; ?>,volumeDefault:<?php echo $video_volume_setting; ?><?php echo $nzs_video_quality; ?>});
